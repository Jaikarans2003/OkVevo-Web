...
function DirectorWorkstation() {
    ...
    // ── Session history ──────────────────────────────────────────────────────────
    const { sessionId, initSession, saveSession, resetSession } = useWorkspaceSession('director', userProfile?.uid ?? null);

    // Persist messages & state whenever they change
    useEffect(() => {
        if (!sessionId || messages.length === 0) return;
        const safeMessages = messages.map(m => ({ role: m.role as 'user' | 'assistant', content: m.content }));
        saveSession(
            { currentState, projectName: project.name },
            safeMessages,
        );
    }, [messages, currentState]);

    // Auto-create session when the user sends their first message (project name)
    useEffect(() => {
        if (sessionId) return;
        if (project.name && messages.length > 1 && userProfile?.uid) {
            initSession(project.name || 'Director Session', { currentState }, []);
        }
    }, [project.name, messages.length]);

    const handleRestoreSession = useCallback((session: WorkspaceSession) => {
        // Reset current flow and reload from session
        resetFlow();
        resetSession();
        // Note: full state restoration would require extending useDirectorFlow
        // For now we just switch context (session messages show in sidebar history)
        initSession(session.title, session.state, []);
    }, [resetFlow, resetSession, initSession]);

    const handleNewSessionDirector = useCallback(() => {
        resetFlow();
        resetSession();
    }, [resetFlow, resetSession]);
    ...
    return (
        <div className="h-screen w-screen bg-black text-[#E0E0E0] font-sans flex flex-col overflow-hidden relative">
            ...
            {/* Body — sidebar + chat */}
            <div className="flex-1 flex relative z-20 overflow-hidden pt-10">
                ...
                {userProfile?.uid && (
                    <div className="relative z-30 h-full">
                        <SessionHistorySidebar
                            ...
                            onNewSession={handleNewSessionDirector}
                            accentColor="orange"
                        />
                    </div>
                )}
