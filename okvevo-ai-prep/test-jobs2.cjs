...
async function run() {
  ...
  for (const user of users.docs) {
    ...
    if (!jobs.empty) {
      ...
      if (jobTarget) {
          const mapped = (jobTarget.assetResults || []).map(r => r.type);
          console.log(`RESULT: ID:${jobTarget.id} | EXP:${jobTarget.expectedAssets} | COMPLETED:${jobTarget.completedAssets} | ARR_LEN:${jobTarget.assetResults?.length || 0} | TYPES:[${mapped.join(',')}]`);
          return; 
      }
    }
  }
